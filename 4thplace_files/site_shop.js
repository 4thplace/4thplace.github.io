				...
				success : function(res){
					if(res.msg == 'SUCCESS'){
						...
						if(type === CONST.CART_TYPE_REGULARLY){
							...
							var order_html = '<a class="btn bg-brand to-order _fade_link _btn_order" '+res.order_btn_onclick+' >'+ getLocalizeString("버튼_정기구독신청", "", "정기구독 신청")+'</a>';
							$shop_cart_list.find('._btn_order').html(order_html);
							$shop_cart_list.find('._social_pay').hide();
							$('#cart_normal_tab').removeClass('active');
							$('#cart_regularly_tab').addClass('active');
						}else{
							currentCartCode = res.cart_code;
							var order_html = '<a class="btn bg-brand to-order _fade_link _btn_order" '+res.order_btn_onclick+' >'+ getLocalizeString("버튼_주문하기", "", "주문하기")+'</a>';
							$shop_cart_list.find('._btn_order').html(order_html);
